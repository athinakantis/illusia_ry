    ...
    createSelector(
        ...
        (item, itemQtyInReservatios, itemQtyInCart): Result => {
            ...
            if ((includeCart) && (overallItemQty === itemQtyInCart))
                return { severity: 'warning', message: 'Max quantity of the item is already in cart', translationKey: 'cart.snackbar.maxQtyError' };

            if (includeCart && availableQuantityOfItem === 0) {
                if (itemQtyInCart === 0) return { severity: 'warning', message: "Item is not available for selected dates", translationKey: 'cart.snackbar.datesUnavailable' };
                else return { severity: 'warning', message: "Max quantity of the item for the dates is already in cart", translationKey: 'cart.snackbar.maxQtyDateError' };
            }

            if (overallItemQty < quantity)
                return { severity: 'warning', message: `Requested bigger quantity than available overall by ${quantity - overallItemQty}`, translationKey: 'cart.snackbar.qtyUnavailable' };
