...
export const checkAvailabilityForItemOnDates = (item_id: string, quantity: number, start_date: string, end_date: string) => createSelector(
    [
        selectItemById(item_id), selectQtyForItemInReservationsByIdInDateRange(item_id, start_date, end_date), selectQtyForItemInCartByIdInDateRange(item_id, start_date, end_date)
    ],
    (item, reservation, itemQtyInCart): Result => {
        ...
        return ((quantity - availableQuantityOfItem) <= 0) ?
            ...
            { severity: 'warning', message: "Not enough quantity for selected dates" };
    }
