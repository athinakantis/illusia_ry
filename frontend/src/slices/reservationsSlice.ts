...
import { getMaxAvailableQtyInRangeForOneItem } from "../utility/OverlappingDates";
...
export const selectQtyForItemInReservationsByIdInDateRange = (id: string, start_date: string, end_date: string) => (state: RootState) => {
    ...
    const maxAvailableQtyInRange = getMaxAvailableQtyInRangeForOneItem(new Date(start_date), new Date(end_date), itemReservations);
